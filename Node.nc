...
implementation{
   ...
   bool packageCheck(pack* Package){
      ...
      for(i = 0; i < call PacketList.size(); i++){
         ...
         if(Temp.src == Package->src && Temp.seq == Package->seq && Temp.dest == Package->dest){    // If packet indentifiers and equal then same packet
            return TRUE;
         }
   ...
   void discoverNeighbors(){
        ...
        for(i = 0; i < size; i++){
            ...
            if(temp.Age > 3){                                // If Neighbor = 3 missing remove
